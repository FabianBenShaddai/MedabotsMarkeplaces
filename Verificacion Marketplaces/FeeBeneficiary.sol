...
contract FeeBeneficiary is Ownable {
    ...
    //function call by factoryPart
    function tranferFoundOfMint(
        uint _amount, 
        IERC20 _token, 
        address _from
    ) public onlyContractsBeneficiary {
        if(address(poolLiquidityAddress)!= address(0)) {
            _token.transferFrom(_from, poolLiquidityAddress, _amount * poolLiquidityMintPercentage / 100);
        }
        if(address(houseFounds)!= address(0)) {
            _token.transferFrom(_from,houseFounds, _amount * houseFoundsPercentage / 100);
        }
    }
